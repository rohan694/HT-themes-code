    ...
    function urna_tbay_post_info_author()
    {
		...
		<div class="author-info">
			...
			<div class="content">
				...
				<a href="<?php echo esc_url( get_author_posts_url( $authordata->ID, $authordata->user_nicename ) ); ?>" class="all-post"><?php esc_html_e('all author posts', 'urna'); ?></a>
			</div>
		<?php
        }
    }
...
if (! function_exists('urna_tbay_render_breadcrumbs')) {
    function urna_tbay_render_breadcrumbs()
    {
        ...
        if (urna_is_Woocommerce_activated() && (is_edit_account_page() || is_add_payment_method_page() || is_lost_password_page() || is_account_page() || is_view_order_page())) {
            $title_bottom = '';
...
if (! function_exists('urna_tbay_render_title')) {
    ...
    {
        if( urna_tbay_is_home_page() ) return;
...
if (! function_exists('urna_tbay_paging_nav')) {
    function urna_tbay_paging_nav()
    {
		...
		<nav class="navigation paging-navigation">
			<h5 class="screen-reader-text hidden"><?php esc_html_e('Posts navigation', 'urna'); ?></h5>
...
if (!function_exists('urna_get_header_id')) {
    function urna_get_header_id()
    {
...
if (!function_exists('urna_get_display_header_builder')) {
    ...
    {
		$id = urna_get_header_id();
		
		return  urna_get_html_custom_post($id);
    }
}
...
if ( ! function_exists( 'urna_get_elementor_post_scripts' ) ) {
	function urna_get_elementor_post_scripts() {
		if( !urna_elementor_is_activated() ) return;
		...
		if ( class_exists( '\Elementor\Core\Files\CSS\Post' ) ) {
			$css_file = new \Elementor\Core\Files\CSS\Post( urna_get_header_id() );
		} elseif ( class_exists( '\Elementor\Post_CSS_File' ) ) {
			$css_file = new \Elementor\Post_CSS_File( urna_get_header_id() );
		}
...
if ( !function_exists('urna_tbay_back_to_top') ) {
    function urna_tbay_back_to_top(){
    ...
    add_action('elementor/theme/after_do_footer', 'urna_tbay_back_to_top', 10);
    add_action('urna_after_do_footer', 'urna_tbay_back_to_top', 10);
}