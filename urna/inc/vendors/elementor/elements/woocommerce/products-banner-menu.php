...
class Urna_Elementor_Products_Banner_Menu extends Urna_Elementor_Carousel_Base
{
    ...
    public function render_item_show_all()
    {
        ...
        if (!empty($categories) && is_object(get_term_by('slug', $categories['0'], 'product_cat')) && is_array($categories) && count($categories) === 1) {
        ...
        <div class="hidden-sm hidden-xs banner">
            <?php if (!empty($banner_link['url'])) : ?>
                <div class="img-banner">
                    <a <?php echo $this->get_render_attribute_string('banner_link'); ?> >
...
$widgets_manager->register(new Urna_Elementor_Products_Banner_Menu());
