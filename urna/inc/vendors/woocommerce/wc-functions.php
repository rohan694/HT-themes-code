...
function urna_woo_remove_wc_breadcrumb()
{
    if (!urna_tbay_get_config('show_product_breadcrumb', false)) {
        remove_action('urna_woo_template_main_before', 'woocommerce_breadcrumb', 20);
    }
}
...
if (!function_exists('urna_tbay_woocommerce_get_display_mode')) {
    function urna_tbay_woocommerce_get_display_mode()
    {
        ...
        if ( !urna_woo_is_vendor_page() && !is_shop() && !is_product_category() && !is_product_tag()) {
            $woo_mode = 'grid';
        }
...
if (!function_exists('urna_tbay_get_woocommerce_layout_configs')) {
    function urna_tbay_get_woocommerce_layout_configs()
    {
        ...
        if (!is_singular('product')) {
            ...
            if (urna_woo_is_mvx_vendor_store()) {
...
if (!function_exists('urna_class_wrapper_start')) {
    function urna_class_wrapper_start()
    {
        ...
        if( !empty( $sidebar_configs['main']['class'] ) ) {
...
if (! function_exists('urna_fnc_more_post_ajax')) {
    ...
    function urna_fnc_more_post_ajax()
    {
        ...
        if ((strpos($category, ',') !== false)) { 
            ...
            $loop = urna_tbay_get_products($categories, $type, $paged, $number_load, '', '', $offset);
        } else {
            if ($category == -1) {
                $loop = urna_tbay_get_products('', $type, $paged, $number_load, '', '', $offset);
            } else {
                $loop = urna_tbay_get_products(array($category), '', $paged, $number_load, '', '', $offset);
            }
        }
    ...
    function urna_woocommerce_quickview()
    {
        if (!empty($_GET['product_id'])) {
            ...
            if ($query->have_posts()) {
                ...
                if( urna_vc_is_activated() ) WPBMap::addAllMappedShortcodes();
...
if (!function_exists('urna_woocommerce_buy_now')) {
    function urna_woocommerce_buy_now()
    {
        ...
        echo '<button class="'. esc_attr($class) .'">'. esc_html__('Buy Now', 'urna') .'</button>';
...
if (!function_exists('urna_tbay_product_layout_style')) {
    function urna_tbay_product_layout_style($type)
    {
        ...
        if( $type === 'v15' && urna_tbay_get_config('enable_woocommerce_quantity_mode', false) ) {
            return $type;
        } elseif( apply_filters('urna_product_layout_mobile', wp_is_mobile() ) ) {
            $type = 'v1';
        }
    ...
    function urna_tbay_the_list_images_review()
    {
        ...
        if (is_array($comments) || is_object($comments)) {
            ...
            foreach ($comments as $comment) {
                ...
                $total_image =  count($image_post_ids);
                $more = '';
                ...
                if (is_array($image_post_ids) || is_object($image_post_ids)) {
                    foreach ($image_post_ids as $image_post_id) {
                        if (! wc_is_valid_url($image_post_id)) {
                            ...
                            $more   = '<div class="more">'. ($total_image - $i) .'+</div>';
                            ...
                            $outputs .= '<li class="review-item">'. $more .'<a class="lightbox-gallery" data-caption="'. esc_attr($caption) .'" data-width="'. esc_attr($width) .'" data-height="'. esc_attr($height) .'"  href="'. esc_url($img_src_open) .'"><img class="review-images"
...
if (! function_exists('urna_woo_is_mvx_vendor_store')) {
    function urna_woo_is_mvx_vendor_store()
    {
        if (! class_exists('MVX') ) {
            return false;
        }
...
if (! function_exists('urna_woocommerce_cart_item_name')) {
    function urna_woocommerce_cart_item_name($name, $cart_item, $cart_item_key)
    {
        ...
        $thumbnail      = $_product->get_image('tbay_photo_reviews_thumbnail_image');
...
if ( ! function_exists( 'urna_is_customize_checkout_payment' ) ) {
    function urna_is_customize_checkout_payment( ) {
        $active = false;

        if( class_exists('WPMultiStepCheckout') || class_exists('Cartflows_Loader')  ) {
            $active = true;
        }
...
if ( ! function_exists( 'urna_woocommerce_checkout_cart_item_quantity_filter' ) ) {
    add_filter( 'woocommerce_checkout_cart_item_quantity', 'urna_woocommerce_checkout_cart_item_quantity_filter', 1, 3 ); 
    function urna_woocommerce_checkout_cart_item_quantity_filter( $html, $cart_item, $cart_item_key ){
        
        if( urna_tbay_get_config('show_checkout_quantity', false) ) {
