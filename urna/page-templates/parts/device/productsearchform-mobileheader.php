<?php if (urna_tbay_get_config('show_searchform') || urna_tbay_get_config('select-header-page') === 'builder'): ?>
	...
	<div class="tbay-search-form tbay-search-mobile">
		    <form action="<?php echo esc_url(home_url('/')); ?>" method="get" data-parents=".topbar-device-mobile" class="searchform urna-ajax-search" data-search-in="<?php echo esc_attr($options); ?>" data-appendto=".search-results-<?php echo esc_attr($_id); ?>" data-thumbnail="<?php echo esc_attr($enable_image); ?>" data-price="<?php echo esc_attr($enable_price); ?>" data-minChars="<?php echo esc_attr($minchars) ?>" data-post-type="<?php echo esc_attr($search_type) ?>" data-count="<?php echo esc_attr($number); ?>">
			<div class="form-group">
				...
				<?php if ($enable_search_category): ?>
					...
					<div class="select-category input-group-addon">
						...
						<?php if (urna_is_Woocommerce_activated() && urna_tbay_get_config('search_type') == 'product') :
                            $args = array(
